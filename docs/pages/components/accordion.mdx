...
import { DefaultAccordion, CautionAccordion, AccordionWithHeading } from "components/examples/Accordion";
...
The toggle button uses the `aria-expanded` attribute to indicate whether the content is visisble (`true`) or not (`false`).
