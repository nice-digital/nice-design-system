...
import { InfoAlert, SuccessAlert, CautionAlert, ErrorAlert,InfoAlertNonIntrusive } from "components/examples/Alert";
...
By default, alerts are set to interrupt the user’s activity but [can be set to ‘polite’](#non-intrusive-alert-option) so the alert is announced without interrupting the user.
...
```
```html HTML
<div class="alert alert--info" data-component="alert--info" role="alert">
	<h3>Info alert</h3>
	<p>Lorem ipsum dolor sit amet, consectetur adipiscing elit.</p>
</div>
```
