...
By default, alerts are set to interrupt the user’s activity but can be set to ‘polite’ so the alert is announced without interrupting the user.[Non-intrusive alert option](#non-intrusive-alert-option)
