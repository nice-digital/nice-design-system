...
You can add `heading` markup to the legend by defining a `headingLevel` value. The value can be set from 1 to 6, corresponding to heading levels h1 through h6.

If no `headingLevel` is provided, no heading markup will be applied.
