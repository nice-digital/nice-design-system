 ...
 * @type       {string}
 */
export const pluginName = "inpagenav";

/**
 * The headings to exclude
 * @type       {string}
 */
export const headingsExclude =
	"[data-no-inpagenav] *, [data-no-inpagenav], .card *, .footer *, .site-footer *, .stacked-nav *, .page-header *";
...
export default class InPageNav {
	...
	constructor(element, options) {
		...
		$(window)
			...
			.on(
				`scroll.${pluginName}.${this.uid}`,
				utils.throttle(() => {
					this.updateNavState(true);
				}, 50)
			)
			.on(
				`resize.${pluginName}.${this.uid}`,
				utils.debounce(() => {
	...
	render() {
		...
		var buildTree = (headings, level = 0) => {
			var arr = [];
	...
	updateNavState(updateHash = false) {
		...
		let activeHref = "#" + activeHeading.id,
			$activeLink = $("a[href='" + activeHref + "']", this.$inpagenav).attr(
				"aria-current",
				"location"
			);
	...
	calculatePosition() {
		let isFixed =
			this.$inpagenav.outerHeight() <= $(window).height() &&
		...
		if (isFixed) {
			...
			if ($footer.length > 0) {
				var footerYPos =
						$(this.options.footerContainer).offset().top -
						$(window).scrollTop(),
	...
	getActiveHeading() {
		...
		$(this.headings).each((i, heading) => {
			let y =
				$(heading).offset().top - scrollTop - this.options.scrollTolerance;
			if (y <= 0) activeHeading = heading;
			else return false;
		});
	 ...
	 */
	generateHeadingId(heading): HTMLHeadingElement {
		if (heading.id) return heading;

		let slug = utils.slugify(heading.textContent || heading.innerText);
		...
		let slugPrefixRegex: RegExp = new RegExp(`^${slug}(-[0-9]+)?`, "i");

		let isSlugPrefixMatch = (i, el) => el.id.match(slugPrefixRegex);

		let max: number = $("[id]").filter(isSlugPrefixMatch).length;
