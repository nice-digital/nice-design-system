...
- [icon font](#icon-font), for when you're working with raw HTML or [in SASS](#usage-in-sass), for writing a bespoke component
...
Alternatively, if you have spacing modifiers enabled (with `$include-spacing-modifiers: true` in SASS) use a class of `mr--c` to put space between the icon and text:
...
const app = express(),
	iconsPath = path.join(__dirname, "./node_modules/@nice-digital/icons/dist/");

app.use("/fonts", express.static(iconsPath));
```
