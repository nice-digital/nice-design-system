...
describe("@nice-digital/nds-button", () => {
	...
	describe("anchors", () => {
		it("renders an a tag when type prop is anchor", () => {
			wrapper = mount(<Button type="anchor">Test</Button>);
			expect(wrapper.find("a")).toBeTruthy();
		});

		it("renders an href attribute from href prop", () => {
			wrapper = mount(
				<Button type="anchor" href="/test">
					Test
				</Button>
			);
	...
	describe("props", () => {
		it("renders type attribute from type prop", () => {
			wrapper = mount(<Button type="submit">Test</Button>);
			expect(wrapper.find("button").prop("type")).toEqual("submit");
		});

		it("calls onClick handler", () => {
			const clickHandler = jest.fn();
			let wrapper = mount(<Button onClick={clickHandler}>Test</Button>);
			wrapper.find("button").simulate("click");
			expect(clickHandler).toHaveBeenCalled();
		});

		it("adds modifier prop to class name", () => {
			let wrapper = mount(<Button modifier="inverse">Test</Button>);
		...
		it("throws error from unsupported modifier", () => {
			...
			expect(() => {
				mount(<Button modifier="invalid">Test</Button>);
			}).toThrow(
				"Expected modifier to be one of 'cta', 'secondary', 'inverse' but found 'invalid'"
			);
		...
		it("renders props as attributes", () => {
			wrapper = mount(
				<Button type="submit" disabled="disabled" modifier="cta">
					txt
				</Button>
			);
